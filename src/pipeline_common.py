"""Shared setup: the dataset, feature scaling, the lookup grid, and byte
storage. Defined once here so every stage agrees on all four.
"""
import numpy as np
...
# Flash cost is classes * GRID_N**2 bytes, so this is a memory decision.
GRID_N = 16

# Below this, the winning class is too weak to trust: answer "unknown".
REJECT_TAU = 0.12

# Plot axis limits: the [0,1] feature space padded 10% each side, so points
# on (or just past) the training range stay clear of the frame.
VIEW_LIM = (-0.1, 1.1)

# Three-way split. Train builds the model (clustering, KDE, discretization);
# validation tunes the design choices (bandwidth, grid); test is spent once,
# on the frozen design, to report an honest number on unseen data.
...
def load_dataset(csv_path=None):
    ...
    sit at well-separated centres so DBSCAN recovers them at a stable eps
...
def normalise_peak(surface):
    ...
    Per class, so a small class is not dwarfed by a large one: the
    classifier compares "how close to this class's own peak am I".
    """
    peak = surface.max()
